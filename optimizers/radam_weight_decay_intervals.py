...
def _multi_tensor_radam(
    ...
    *,
    ...
    for ((
        ...
        if weight_decay != 0:
            if decoupled_weight_decay:
                for i,par in enumerate(grouped_params):
                      wd=torch.where(torch.abs(grouped_grads[i]) < 1e-4, 0.1,
                      torch.where(torch.abs(grouped_grads[i]) < 1e-3, 0.05,
                                 torch.where(torch.abs(grouped_grads[i]) < 1e-2, 0.01,
                                            torch.where(torch.abs(grouped_grads[i]) < 1e-1, 0.005, 0.001))))
                      par.mul_(1 - lr*wd)
            else:
