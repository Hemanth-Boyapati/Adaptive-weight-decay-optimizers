...
class AdamW(Optimizer):
    def __init__(
        ...
        eps: float = 1e-8,
        weight_decay1: float = 1e-2,
        weight_decay2: float = 1e-2,
        amsgrad: bool = False,
        *,
        maximize: bool = False,
        ...
        if not 0.0 <= weight_decay1:
            raise ValueError(f"Invalid weight_decay value: {weight_decay1}")
        if not 0.0 <= weight_decay2:
            raise ValueError(f"Invalid weight_decay value: {weight_decay2}")
        defaults = dict(lr=lr, betas=betas, eps=eps, 
                        weight_decay1=weight_decay1,weight_decay2=weight_decay2, amsgrad=amsgrad, foreach=foreach,
...
def _single_tensor_adamw(
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        wd=torch.where(torch.abs(grad) < 1e-4, 0.1,
                      torch.where(torch.abs(grad) < 1e-3, 0.05,
                                 torch.where(torch.abs(grad) < 1e-2, 0.01,
                                            torch.where(torch.abs(grad) < 1e-1, 0.005, 0.001))))
        param.mul_(1 - lr * wd)
    ...
    *,
    ...
    for ((
        ...
        if weight_decay1 != 0 and weight_decay2 !=0:
            for i,par in enumerate(device_params):
                      wd=torch.where(torch.abs(device_grads[i]) < 1e-4, 0.1,
                      torch.where(torch.abs(device_grads[i]) < 1e-3, 0.05,
                                 torch.where(torch.abs(device_grads[i]) < 1e-2, 0.01,
                                            torch.where(torch.abs(device_grads[i]) < 1e-1, 0.005, 0.001))))
                      par.mul_(1 - lr * wd)
        # Decay the first and second moment running average coefficient
