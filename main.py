...
best_acc1 = 0
MODELS = ['vit', 'swin', 'pit', 'cait', 't2t', 'swinv2']


def init_parser():
    parser = argparse.ArgumentParser(description='CIFAR100 quick training script')
    ...
    parser.add_argument('--epoch_start', default=0, type=int, metavar='N', help='starting epoch')
    ...
    parser.add_argument('--lr1', default=0.003, type=float, help='initial learning rate')
    ...
    parser.add_argument('--enable_aug', action='store_false', help='disable augmentation policies for training')
...
def main(args):
    ...
    logger.debug(f'Initial learning rate1: {args.lr1:.6f}')
    logger.debug(f'Initial learning rate2: {args.lr2:.6f}')
    logger.debug(f'Weight decay: {args.weight_decay:.6f}')
    ...
    parameters1 = []
    parameters2 = []
    parameters1 += [{'params': [p for n, p in model.named_parameters() if ('suppress'not in n and  'thres' not in n)   and p.requires_grad], 'lr': args.lr1}]
    parameters2 += [{'params': [p for n, p in model.named_parameters() if ('suppress'in n or 'thres' in n)   and p.requires_grad], 'lr': args.lr2}]
    optimizer1 = RAdam(parameters1, weight_decay=5e-2,decoupled_weight_decay=True)
    scheduler1 = build_scheduler(args, optimizer1, len(train_loader), args.lr1)
    optimizer2 = RAdam(parameters2, weight_decay=5e-2,decoupled_weight_decay=True)
    scheduler2 = build_scheduler(args, optimizer2, len(train_loader), args.lr2)
    ...
    if args.resume:
        ...
        args.epoch_start=checkpoint['epoch'] + 1
    ...
    for epoch in range(args.epoch_start,args.epochs):
        ...
        lr2 = optimizer2.param_groups[0]["lr"]
        print()
        print('Learning rates: ',lr1,' ',lr2)
    #     with open("gradient_stats.csv", 'a', newline='') as file:
    # # Create a CSV writer object
    #       csv_writer = csv.writer(file)
    # # Write the new line to the CSV file
    #       csv_writer.writerow([0,0,epoch])
    # If it's the first epoch, write the header
        lr1, lr2 = train(train_loader, model, criterion, optimizer1,optimizer2, epoch, scheduler1,scheduler2, args)
        acc1 = validate(val_loader, model, criterion, lr1, lr2, args, epoch=epoch)
        torch.save({
        ...
        writer.add_scalar("Learning Rate1", lr1, epoch) #, "Learning Rate2", lr2, epoch)
...
def train(train_loader, model, criterion, optimizer1,optimizer2, epoch, scheduler1,scheduler2,  args):
    model.train()
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.print_freq >= 0 and i % args.print_freq == 0:
            ...
            progress_bar(i, len(train_loader),f'[Epoch {epoch+1}/{args.epochs}][T][{i}]   Loss: {avg_loss:.4e}   Top-1: {avg_acc1:6.2f}   LR: {lr1:.7f}  LR2: {lr2:.7f}'+' '*10)
...
def validate(val_loader, model, criterion, lr1, lr2, args, epoch=None):
    model.eval()
    loss_val, acc1_val = 0, 0
    n = 0
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            if args.print_freq >= 0 and i % args.print_freq == 0:
                avg_loss, avg_acc1 = (loss_val / n), (acc1_val / n)
                progress_bar(i, len(val_loader), f'[Epoch {epoch+1}][V][{i}]   Loss: {avg_loss:.4e}   Top-1: {avg_acc1:6.2f}   LR: {lr1:.6f} LR: {lr2:.6f}')
    print()        
...
if __name__ == '__main__':
    ...
    model_name += f"-{args.tag}-{args.dataset}-LR[{args.lr1}]-LR[{args.lr2}]-Seed{args.seed}-Learnable-S{args.suppress}-T{args.thres}"
