...
def dataload(args, augmentations, normalize, data_info):
    if args.dataset == 'CIFAR10':
        train_dataset = datasets.CIFAR10(
            root=os.path.join(args.data_path,'cifar-10'), train=True, transform=augmentations,download=True)
        val_dataset = datasets.CIFAR10(
            root=os.path.join(args.data_path,'cifar-10'), train=False, transform=transforms.Compose([
        ...
        train_dataset = datasets.CIFAR100(
            root=os.path.join(args.data_path,'cifar-100'), train=True,transform=augmentations,download=True)
        val_dataset = datasets.CIFAR100(
            root=os.path.join(args.data_path,'cifar-100'), train=False,transform=transforms.Compose([
            transforms.Resize(data_info['img_size']),
            transforms.ToTensor(),
            *normalize]),download=True)
        ...
        train_dataset = datasets.SVHN(
            root=os.path.join(args.data_path,'svhn'), split='train', transform=augmentations)
        val_dataset = datasets.SVHN(
            root=os.path.join(args.data_path,'svhn'), split='test', transform=transforms.Compose([
            transforms.Resize(data_info['img_size']),
            transforms.ToTensor(),
            *normalize]))
